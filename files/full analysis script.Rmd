...
points <- point_estimate(means_Congruency) %>% 
  tidyr::pivot_longer(-Parameter, names_to = "Est")

p_means + 
  stat_slab() + 
  geom_point(aes(Parameter, value, color = Est), data = points) + 
  geom_segment(aes(Parameter, value, color = Est,
                   xend = stat(x) + 0.9, yend = value), 
               data = points)
```
