...
To fit a Bayesian model (= to estimate the parameters in your model), you need:
...
Notes:
  ...
  - For `Congruency` I've used ***orthonormal* dummy-coding**. This is important, but `r emo::ji("hourglass_not_done")`! Read more about that [here](https://easystats.github.io/bayestestR/articles/bayes_factors.html#contr_bayes).
...
.footnote[
But you can find them, and more, in [the full analysis script](files/full%20analysis%20script.nb.html)...
]
...
For this we first need to obtain a model that represents purly our priors - before that have been updated by the data - which we will then *compare* to the results from our updated (posterior) model.
...
For *covariates*, we can present posterior slopes, not just the median slope.
...
ggplot(BFplot_data$plot_data, aes(x, ind, fill = Distribution, color = Distribution)) +
  geom_slab(aes(thickness = y), alpha = 0.4) +
  annotate("rect", fill = "red", alpha = 0.4,
           xmin = -40, xmax = 40, ymin = -Inf, ymax = Inf) +
  scale_fill_manual(NULL, 
                    values = c(prior = "blue", posterior = "darkgreen"), 
                    aesthetics = c("fill", "color")) + 
  scale_x_continuous(expand = c(0, 0)) + 
