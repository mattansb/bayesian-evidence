...
style_duo_accent(
  primary_color = "#18a1dc",
  secondary_color = "#e88c8c",
  # Header
...
.right[
Presented at 23/02/2021 <br> .small[(updated `r format(Sys.Date(), "%d/%m/%Y")`)]
`r fa("twitter",,"black")` [.black[@mattansb]](https://twitter.com/mattansb) <br>
`r fa("github",,"black")` [.black[@mattansb]](https://github.com/mattansb) <br>
]
...
```{r normal_prior1, fig.width = 16, fig.height = 2.3}
dPrior1 <- function(x) rep(.Machine$double.xmin, length(x))
plot_bayes(dPrior1, dLikelihood, header = TRUE)
```
...
- <b>.green[Pros]</b>: Resembles the *p*-value.
...
ggplot(diff_draws, aes(.value, contrast)) +
  stat_slab(aes(fill = stat(cut_cdf_qi(cdf, .width = c(0.05, 1))))) + 
  stat_slab(aes(alpha = stat(dplyr::between(x, -3, 3))), fill = "red") + 
  scale_alpha_manual(values = c(0, 1), guide = NULL) + 
  scale_fill_manual(NULL, values = c("purple4", "gray"),
                    labels = c("MAP", "null"),
...
- <b>.green[Pros]</b>: Closely related to LRT tests, and so also resembles the *p*-value.
...
ggplot(diff_draws, aes(.value, contrast)) +
  stat_slab(aes(fill = stat(cut_cdf_qi(cdf, .width = c(0.89, 1)))), color = "gray") + 
...
It is very improbable that the Interference effect is very small. 

--

There is also about a 30% that among 4 year olds, there is no Facilitation effect - ([though not very conclusive](https://easystats.github.io/bayestestR/articles/guidelines.html#significance)) we are supporting the null!
...
Posteriors show us the total - but with any set of priors we can "engeneer" any posterior we want.  

--

Also, posterior based methods generally treat 0 (or any null value) like any other - that is, we are not strictly asking comparative questiosn. Or in other words, we're not conducting hypothesis *testing*.  


- Other Bayes Factors....
  - Bayes factor for dividing hypotheses (add link)
  - Restricted Model Bayes factors (add link)
...
Recommendations - https://easystats.github.io/bayestestR/articles/guidelines.html
...
- `r fa("twitter",,"#000000")` [@mattansb](https://twitter.com/mattansb)

- `r fa("github",,"#000000")` [@mattansb](https://github.com/mattansb)

- `r fa("rss",,"#000000")` [Blog](https://shouldbewriting.netlify.com/)
]
...
.small[
[`easystats/`**`bayestestR`**](https://easystats.github.io/bayestestR) core team:
...
background-image: url(img/boyfriend.jpg)
class: right, bottom
