...
**Any** measure the quantifies this `r emo::ji("backhand_index_pointing_up")` is a Bayes factor.
...
levels(BFplot_data$d_points$ind) <- rev(levels(BFplot_data$plot_data$ind))
levels(BFplot_data$plot_data$ind) <- rev(levels(BFplot_data$plot_data$ind))

ggplot(BFplot_data$plot_data, aes(x, ind, fill = Distribution, color = Distribution)) +
  geom_slab(aes(thickness = y), alpha = 0.4) +
  geom_point(aes(x = x, y = 135 * y + as.numeric(factor(ind)), fill = Distribution),
